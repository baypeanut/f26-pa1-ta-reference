...
static GBitmap handle_proc(const GDrawRec& rec, const char path[]) {
    GBitmap bitmap(rec.fSize);

    auto canvas = GCreateCanvas(bitmap);
    if (!canvas) {
        fprintf(stderr, "failed to create canvas for [%d %d] %s\n",
                rec.fSize.width, rec.fSize.height, rec.fName);
        return bitmap;  // empty
    }
...
static void copy_with_substitution(const char src[], FILE* dst, const char* subst[], size_t count) {
    for (size_t i = 0; src[i]; ++i) {
        unsigned char c = src[i];
        if (c == '%') { // string substitution
            size_t index = src[++i] - '0';    // make it a digit
            assert(index < count);
            fprintf(dst, "%s", subst[index]);
        } else {
            fprintf(dst, "%c", c);
        }
    }
...
static void handle_something(FILE* f, std::string prefix, const char dir[], int index) {
    ...
    if (canvas) {
        ...
        if (f) {
            ...
            const char* substr[] = { name, title.c_str() };
            copy_with_substitution(gItemStr, f, substr, std::size(substr));
...
static int max_name_len() {
    size_t len = 0;
    ImageRegistrant::Visit([&](const auto& rec) {
        len = std::max(len, strlen(rec.fName));
    });
    return (int)len;
}
...
int main_image(int argc, const char* argv[]) {
    ImageRegistrant::Sort([](const GDrawRec& a, const GDrawRec& b) {
        return a.fPA < b.fPA;
    });
    ...
    double counter = 0;
    ImageRegistrant::Visit([&](const auto& rec, size_t index) {
        double weight = 1 << (rec.fPA - 1);
        weight /= gPACounts[rec.fPA];

        std::string path(root);
        // we add a prefix to make it easy for 'make clean' to remove these, but not other images
        if (append_pa_prefix) {
            path += make_image_prefix(rec.fPA);
        }
        path += rec.fName;
        path += ".png";

        bool something = strncmp(rec.fName, "something_", strlen("something_")) == 0;
        if (!something) {
            counter += weight;
        }
        ...
        if (verbose && !something) {
            printf("image: [%2zu] %*s", index, maxNameLen, path.c_str());
        }
        
        GBitmap testBM = handle_proc(rec, path.c_str());
        ...
        if (expected && !something) {
            ...
            if (auto expectedBM = GBitmap::ReadFromFile(exp_path.c_str())) {
                double correct = compare(testBM, *expectedBM, tolerance, verbose);
                if (correct < 1 && diffFile != NULL) {
                    add_diff_to_file(diffFile, testBM, *expectedBM, diffDir, rec.fName);
                }
